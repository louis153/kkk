 ...
 * @return
 */
@Component("binded_gto_amount")
public class GtoBindedStrategy implements IMethodStrategy {
	
	public static class Tab_Invitation_BindDetail {
		public String gto; // GTO数
	}
	
	protected final transient static Logger logger = LoggerFactory.getLogger(GtoBindedStrategy.class);
	@Autowired
	...
	public String doJsonMethod(Request_LtGameLogic request_LtGameLogic, JSONObject jsonRequest) throws Exception {
		...
		logger.info("binded_gto_amount开始调用被绑定邀请码获得GTO数量接口doJsonMethod------>");
		...
		try {	
			...
			if(tab_invitation_bindList.size()>0){
				...
				tab_Invitation_BindDetail.gto = StringUtil.removeEndZero(tab_invitation_bind.getBe_bind_fee_total().toString());
